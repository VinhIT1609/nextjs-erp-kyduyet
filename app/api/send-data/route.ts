...
export async function POST(req: Request) {
  try {
    ...
    if (!Array.isArray(contractsArray) || contractsArray.length === 0) {
      throw new Error("Dữ liệu gửi lên phải là một danh sách (Array) và không được rỗng.");
    }
    ...
    for (let i = 0; i < contractsArray.length; i++) {
      ...
      if (!item.contract_id || typeof item.contract_id !== "string") {
        throw new Error(`Dòng số ${rowNum}: Thiếu trường 'contract_id' hoặc sai định dạng.`);
      }
      if (!item.email || typeof item.email !== "string" || !item.email.includes("@")) {
        throw new Error(`Dòng số ${rowNum}: Trường 'email' không hợp lệ hoặc thiếu ký tự @.`);
      }
      if (!item.employee_name || typeof item.employee_name !== "string") {
        throw new Error(`Dòng số ${rowNum}: Trường 'employee_name' bắt buộc là chuỗi văn bản.`);
      }
      if (!item.employee_no || typeof item.employee_no !== "string") {
        throw new Error(`Dòng số ${rowNum}: Trường 'employee_no' bắt buộc là chuỗi văn bản.`);
      }
      if (!item.contract_no || typeof item.contract_no !== "string") {
        throw new Error(`Dòng số ${rowNum}: Trường 'contract_no' bắt buộc là chuỗi văn bản.`);
      }
      if (!item.pdf_file || typeof item.pdf_file !== "string") {
        throw new Error(`Dòng số ${rowNum}: Trường 'pdf_file' bắt buộc chứa đường dẫn file dạng chuỗi.`);
      }
    }
    ...
    for (const item of contractsArray) {
      try {
        ...
        const checkQuery = "SELECT STATUS FROM PER_CONTRACT WHERE FACT_NO = '0000' AND PNL_NO = :1 AND CONTRACT_NO = :2 AND STATUS = 'RECEIVED'";
        const checkResult = await executeSql(checkQuery, [item.employee_no, item.contract_id]);
        ...
        if (existingContract) {
          ...
          if (currentStatus === "RECEIVED") {
            console.log(`[BỎ QUA] Mã HĐ ${item.contract_id} đang trong tiến trình gửi thư cũ, từ chối lệnh gửi đè.`);
            totalSkipped++;
            continue; 
          }
          ...
          if (isUserWantResend) {
            console.log(`[GỬI LẠI] Đồng ý kích hoạt lại mail mới cho mã HĐ: ${item.contract_id}`);
            totalResent++;
          } 
          // TRƯỜNG HỢP 3: Bị trùng ID do API trùng lặp ngẫu nhiên thông thường (is_resend = false)
          else {
            console.log(`[CHẶN TRÙNG LẶP] Bỏ qua mã HĐ ${item.contract_id} do đã tồn tại trong cơ sở dữ liệu Oracle.`);
            totalSkipped++;
            continue; 
          }
        } else {
        ...
        const mergeQuery = "UPDATE PER_CONTRACT SET STATUS ='RECEIVED' WHERE FACT_NO = '0000' AND PNL_NO = :1 AND CONTRACT_NO = :2";
        ...
        setTimeout(async () => {
          try {
            const workerResponse = await fetch(`${origin}/api/send-mail`, {
              method: "POST",
            ...
            if (!workerResponse.ok) {
              const workerErrorData = await workerResponse.json().catch(() => ({}));
              console.error(`[Lỗi Worker ngầm tại ID ${item.contract_id}]:`, workerErrorData.error);
            }
          } catch (fetchErr: any) {
            console.error(`[Lỗi kết nối Worker ngầm cho ID ${item.contract_id}]:`, fetchErr.message);
          }
        }, 0);

      } catch (innerError: any) {
        console.error(`❌ Sự cố khi ghi nhận Oracle cho mã ${item.contract_id}:`, innerError.message);
      }
    }

    console.log(`=== ĐỒNG BỘ ORACLE HOÀN TẤT: Nhận mới: ${totalProcessed}, Gửi lại: ${totalResent}, Chặn trùng: ${totalSkipped} ===`);
